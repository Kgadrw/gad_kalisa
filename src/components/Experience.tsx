...
import { Laptop, Factory, User, CalendarDays } from "lucide-react"; // Importing icons from lucide-react
...
const Experience = () => {
  ...
  return (
    <section className="relative mx-auto px-32 py-12 border-b border-gray-300 bg-white">
      <h2 className="text-6xl font-[Poppins] font-bold text-gray-900 mb-8 text-left">Work Experience</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-4">
          {experiences.map((exp, index) => (
            <div 
              key={index} 
              className={`cursor-pointer p-4 border border-gray-200 rounded-lg transition duration-300 ease-in-out ${selectedExperience === index ? 'bg-gray-300 text-gray-900 font-[Poppins]' : 'bg-white hover:bg-gray-200 font-[Poppins]'}`} 
              onClick={() => setSelectedExperience(index)}
            >
              <div className="flex items-center space-x-3">
                {index === 0 && <Laptop className="text-2xl text-gray-900" />}
                {index === 1 && <Factory className="text-2xl text-gray-900" />}
                {index === 2 && <User className="text-2xl text-gray-900" />}
        ...
        <div className="col-span-2">
          {selectedExperience !== null && (
            ...
            >
              <h3 className="text-2xl font-bold font-[Poppins] text-gray-900 mb-2">{experiences[selectedExperience].role}</h3>
              <p className="text-gray-600 font-[Poppins] mb-4 italic">
