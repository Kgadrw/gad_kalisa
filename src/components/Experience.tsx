...
const Experience = () => {
  ...
  return (
    <section
      className={`relative mx-auto px-6 sm:px-12 md:px-24 lg:px-32 py-12  ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-700'}`}
    >
      <h2 className={`text-4xl sm:text-5xl md:text-6xl font-[Poppins] font-bold ${darkMode ? 'text-white' : 'text-gray-700'} mb-8 text-center md:text-left`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-4">
          {experiences.map((exp, index) => (
            <div 
              key={index} 
              className={`cursor-pointer p-4 border ${darkMode ? 'border-gray-800 text-gray-400' : 'border-blue-400 '} shadow-md rounded-lg transition duration-300 ease-in-out ${selectedExperience === index ? 'bg-blue-400 text-gray-400 font-[Poppins]' : `${darkMode ? 'bg-gray-900 hover:bg-gray-700 ' : 'bg-white hover:bg-gray-200 '} font-[Poppins]`}`} 
              onClick={() => setSelectedExperience(index)}
            >
              <div className="flex items-center space-x-3">
                {index === 0 && <Laptop className={`text-2xl ${darkMode ? 'text-white' : 'text-gray-700'}`} />}
                {index === 1 && <Factory className={`text-2xl ${darkMode ? 'text-white' : 'text-gray-700'}`} />}
                {index === 2 && <User className={`text-2xl ${darkMode ? 'text-white' : 'text-gray-700'}`} />}
                <h3 className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-600'}`}>{exp.company}</h3>
              </div>
            </div>
          ))}
        </div>
        <div className="col-span-2">
          {selectedExperience !== null && (
            <motion.div 
              ...
              className={`p-6 border rounded-lg ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}
            >
